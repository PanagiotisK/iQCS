...
def draw_quantum_state(final_state, num_qubits):
    return str("\n".join(["%.3f" % np.round(x.real, 4) + (' - ' if x.imag < 0 else ' + ' ) + "%.3f" % np.round(x.imag, 4) + 'j' + '\t|' + (format(idx, f'0{num_qubits}b')) + '\u27E9' for idx,x in enumerate(final_state)]))
...
def draw_circuit(instructions, num_qubits):
    ...
    max_width = 12  # Fixed width to prevent excessive stretching
    num_gates = len(instructions.split("\n"))
    width = min(max_width, num_gates + 1)  # Scale width but limit expansion
    ...
    ax.set_ylim(-num_qubits * 0.5, num_qubits * 1.3)
    ax.set_xticks([])
...
def process_quantum_instructions(instructions: str, debug: bool = False):
    ...
    try:
        ...
        formatted_quantum_state = draw_quantum_state(final_state, num_qubits)

        img_html = draw_probability_plot(final_state, num_qubits)

        circuit_img = draw_circuit(instructions, num_qubits)

        return formatted_quantum_state, img_html, circuit_img, error_message, debug_info
    except Exception as e:
        traceback.print_exc()
...
iface = gr.Interface(
    ...
    description="Enter instructions to manipulate a quantum circuit and visualize the state.<br>"+
                "Valid quantum instructions (e.g., I q3, H q0, CX q0 q1, CCX q0 q1 q2).<br>"+
                "NOTE: One instruction per line."
)
